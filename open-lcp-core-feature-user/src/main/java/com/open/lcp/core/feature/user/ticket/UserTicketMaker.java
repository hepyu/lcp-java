...
public class UserTicketMaker {
	...
	public static String makeTicket(int appId, long userId) {
		...
		return CacheDBSEncoder.deciToB69(deci);
	}

	public static String makeTicket(CoreUserType userType, int appId, long userId) {
		if (userType == null || userType == CoreUserType.admin) {
		...
		final String deci = String.format("%s%s%s", prefix, appId, userId);
		String ticket = CacheDBSEncoder.deciToB69(deci);
	...
	public static UserTicket toUserTicket(String ticket) {
		final String deci = CacheDBSEncoder.b69ToDeci(ticket);
		...
		case 2:// 支持账号类型
			...
			ut.userType = CoreUserType.get(type);
	...
	public static long fromKey(String key) {
		String b69 = CacheDBSEncoder.b69ToDeci(key);
	...
	public static boolean isUser(String t) {
		if (StringUtils.isNotEmpty(t)) {
			...
			CoreUserType userType = userTicket != null ? userTicket.getUserType() : null;
			if (userType != null && userType == CoreUserType.user) {
				return true;
			}
		}
		return false;
