...
import com.open.lcp.framework.core.api.listener.CommandListener;
...
public class ApiFacadeCommand extends AbstractApiCommand implements InitializingBean {
	...
	public ApiResult onExecute(ApiCommandContext ctx) {
		...
		try {
			...
			LcpUtils.rpcTimeCost(execBeginTime, ctx.getMethodName());
		}
	...
	protected void checkReqModel(Object reqModel, LcpReq lcpReq, ApiCommandContext ctx) throws LcpApiException {
		...
		if (result != null && result.getErrorType() != ErrorType.Pass) {
			logger.warn(String.format(SF_REQ_ERROR, ctx.getUserId(), ctx.getSig(), ctx.getMethodName(),
					result.toMessage(), LcpFieldLoadHolder.toString(result.getValue())));
	...
	protected void checkRespModel(Object respModel, ApiCommandContext ctx) throws LcpApiException {
		...
		if (result != null && result.getErrorType() != ErrorType.Pass) {
			logger.warn(String.format(SF_RESP_ERROR//
					...
					, ctx.getMethodName()//
					, result.toMessage()//
					, LcpFieldLoadHolder.toString(result.getValue())));
			throw EXP_ERR_RESP;
		}
	}
