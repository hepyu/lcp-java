...
import com.open.lcp.core.dao.entity.AppInfoEntity;
...
public class ApiCommandContext implements CommandContext {
	...
	/** 客户端接入授权信息 */
	private AppInfoEntity mcpAppInfo;
	...
	private AppInitInfoEntity appInitData = null;

	private byte[] aesKey;
	private byte[] octetBody;

	public ApiCommandContext(long beginTime, AppInfo mcpAppInfo, Map<String, String> stringParams,
		...
		this.appVersion = stringParams.get(McpConstants.PARAM_APP_VERSION);
		this.aesKey = DigestUtils.md5(this.mcpAppInfo.getAppSecretKey());
	...
	@Override
	public AppInfo getMcpAppInfo() {
		return mcpAppInfo;
	}

	public void setMcpAppInfo(AppInfo mcpAppInfo) {
		this.mcpAppInfo = mcpAppInfo;
	}
	...
	public String toString() {
		return "ApiCommandContext [beginTime=" + beginTime + ", methodName=" + methodName + ", userId=" + userId
				+ ", mcpAppInfo=" + mcpAppInfo + ", deviceId=" + deviceId + ", secretKey=" + secretKey + ", ticket="
				+ ticket + ", sig=" + sig + ", stringParams=" + stringParams + ", binaryParams=" + binaryParams + "]";
	}
