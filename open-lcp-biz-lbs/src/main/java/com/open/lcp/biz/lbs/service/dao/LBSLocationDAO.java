...
public interface LBSLocationDAO {
	...
	@SQL("INSERT INTO lbs_location(" + SQL_SELECT_PART
	...
	@SQL("UPDATE lbs_location SET description=:2 WHERE location_code=:1")
	public int updateLocationDesc(long locationCode, String description);

	@SQL("SELECT " + SQL_SELECT_PART + " FROM lbs_location WHERE location_code>:1 ORDER BY location_code ASC LIMIT :2")
	public List<LBSLocationEntity> listLocationCoordinate(long startLocationCode, int pageSize);

	@SQL("UPDATE lbs_location SET location_code=:1.locationCode, city_code=:1.cityCode, city_name=:1.cityName, description=:1.description WHERE location_name=:1.locationName")
	public int updateForWashData(LBSLocationEntity locationCoordinate);

	@SQL("SELECT location_code, location_name, description FROM lbs_location WHERE city_code=:1 ")
	public List<LBSLocationEntity> listAll(long cityCode);

	@SQL("SELECT " + SQL_SELECT_PART + " FROM lbs_location WHERE location_code=:1")
	public LBSLocationEntity getLocationCoordinate(long locationCode);

	@SQL("DELETE FROM lbs_location WHERE location_code=:1")
